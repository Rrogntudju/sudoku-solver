...
impl Sudoku {
    pub fn new() -> Self {
        ...
        let mut units = HashMap::<String, Vec<Vec<String>>, RandomState>::with_capacity_and_hasher(
            81,
            RandomState::default(),
        );
        for s in &squares {
        ...
        let mut peers = HashMap::<String, Vec<String>, RandomState>::with_capacity_and_hasher(
            81,
            RandomState::default(),
        );
        for s in &squares {
    ...
    fn grid_values(&self, grid: &str) -> PuzzleResult<HashMap<String, Vec<char>, RandomState>> {
        ...
        if grid_chars.len() == 81 {
            let mut grid_values =
                HashMap::<String, Vec<char>, RandomState>::with_capacity_and_hasher(
                    81,
                    RandomState::default(),
                );
    ...
    fn parse_grid(&self, grid: &str) -> PuzzleResult<HashMap<String, Vec<char>, RandomState>> {
        ...
        let mut values = HashMap::<String, Vec<char>, RandomState>::with_capacity_and_hasher(
            81,
            RandomState::default(),
        );
    ...
    fn assign(
        &self,
        values: &mut HashMap<String, Vec<char>, RandomState>,
        s: &str,
        d: &char,
    ) -> bool {
    ...
    fn search(
        &self,
        values: HashMap<String, Vec<char>, RandomState>,
    ) -> PuzzleResult<HashMap<String, Vec<char>, RandomState>> {
    ...
    pub fn display(grid: &str) -> PuzzleResult<Vec<String>> {
        let grid_chars: Vec<char> = grid
            .chars()
            .filter(|c| "123456789.0".contains(*c))
            .collect();
        if grid_chars.len() == 81 {
            let width = 2;
            let sep = ["-"; 3]
